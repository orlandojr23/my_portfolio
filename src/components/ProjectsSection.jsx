import { PROJECTS } from '@/config/projects';

export function ProjectsSection() {
  return (
    <div className="space-y-6 sm:space-y-8">
      <div>
        <h2 className="font-bold text-xl sm:text-2xl tracking-tight text-foreground">
          Things I've Built
        </h2>
      </div>

      <div className="space-y-8 sm:space-y-10">
        {PROJECTS.map((project) => (
          <div key={project.id} className="space-y-2.5">
            {/* Header: Title and Date */}
            <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1">
              <h3 className="font-mono text-[11px] sm:text-xs font-semibold tracking-widest text-foreground uppercase">
                {project.title}
              </h3>
              <span className="font-mono text-[10px] sm:text-[11px] text-muted-foreground tracking-widest uppercase">
                {project.period.start}{' '}
                {project.period.end ? `- ${project.period.end}` : '- Present'}
              </span>
            </div>

            {/* Description */}
            <p className="text-foreground/80 text-sm md:text-base leading-relaxed whitespace-pre-line">
              {project.description || project.shortDescription}
            </p>

            {/* Skills */}
            {project.skills && project.skills.length > 0 && (
              <p className="text-muted-foreground text-sm leading-relaxed">
                {project.skills.join(', ')}
              </p>
            )}

            {/* Links */}
            <div className="flex flex-wrap items-center gap-4 pt-1 font-mono text-[11px] sm:text-xs">
              {project.github && (
                <a
                  href={project.github}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-center gap-1.5 text-foreground hover:underline underline-offset-4 decoration-border hover:decoration-foreground transition-colors w-fit"
                >
                  <svg className="size-3.5 fill-current opacity-80 group-hover:opacity-100 transition-opacity" viewBox="0 0 24 24">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                  </svg>
                  GitHub ↗
                </a>
              )}
              {project.link && !project.inDevelopment && (
                <a
                  href={project.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-foreground hover:underline underline-offset-4 decoration-border hover:decoration-foreground transition-colors"
                >
                  Live ↗
                </a>
              )}
              {project.link && project.inDevelopment && (
                <span className="text-muted-foreground/50 uppercase tracking-widest">
                  In Development
                </span>
              )}
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
